...
class CLI {
    ...
    this.svgPrompts = [
      ...
      },
      {
        ...
        validate: (name) => {
          ...
          let temp = parseFloat(name.replace("px",""));
          if (isNaN(temp)) {
      ...
      },
      {
        type: 'input',
        name: 'border-radius',
        message: 'Please enter a border radius (min=0; max=200):',
        validate: function (name) {
          let temp = parseFloat(name.replace("px",""));
          if (isNaN(temp) || (temp) > 200 || (temp) < 0 ) {
    ...
    this.textPrompts = [
      {
        ...
        validate: function (name) {
          if (name.length > 3 || name.length < 1) {
            return `Please enter a text (max 3 characters)`;
          } else {
            return true;
          }
        }
  ...
  addFilter(name) {
    // automatically adds the "px" at the end of user input requiring pixels
    let temp = parseFloat(name.replace("px",""));
    if (isNaN(temp)) {
      return(name);
    } else {
